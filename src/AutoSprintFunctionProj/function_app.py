...
def create_new_sprint(project_id, project_name, team_id, team_name, last_iteration_finish_date, last_iteration_num, sprint_length):
...
def main():
    ...
    logging.info(f"[AutoSprint] Execution started")
        ...
        for team in project["teams"]:
            ...
            if (team["last_iteration"] is not None):
                ...
                if time_difference <= datetime.timedelta(days=1):
                    ...
                    create_new_sprint(
                        project_id, project_name, team_id, team_name, last_iteration_finish_date, last_iteration_num, project_sprint_length)
            else:
                filtered_projects.append(project)
...
@app.function_name(name="SprintChecker")
@app.timer_trigger(schedule="0 0 12 * * *", arg_name="myTimer", run_on_startup=True,
                   use_monitor=False)
def sprint_checker(myTimer: func.TimerRequest) -> None:
